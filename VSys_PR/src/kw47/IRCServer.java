...
public class IRCServer implements Runnable, Actor {
	...
	private String formatPrivateMessage(Client sender, Message m) {
		return String.format("%s PRIVMSG %s :%s", sender.getFull(), m.getArgs()[0], m.getText());
	}
	...
	private void notice(Client sender, Message m) {
		...
		if (clients.containsKey(target)) {
			clients.get(target).sendMessage(String.format("%s NOTICE %s :%s", sender.getFull(), target, m.getText()));
		}
	}
